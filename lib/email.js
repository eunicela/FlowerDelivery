...
const REPLY_TO_EMAIL = process.env.REPLY_TO_EMAIL || 'Eflowerwholesale@gmail.com';

const PICKUP_LOCATION = `San Francisco Flower Market on the 1st Floor – Market Hall (Stall 03), at 901 16th Street (parking entrance at 80 Mississippi St), San Francisco, CA 94107`;
const PICKUP_HOURS = `Pickup hours are between 8:00am and 12:30pm on your selected pickup date.`;
            ...
            <div style="padding: 30px;">
              ...
              <div style="background-color: #f5f0e8; border-radius: 8px; padding: 20px; margin-bottom: 30px;">
                ...
                <table style="width: 100%; font-size: 14px; color: #3a3a3a;">
                  ...
                  <tr>
                    <td style="padding: 8px 0; color: #666;">${isPickup ? 'Pickup Date:' : 'Delivery Date:'}</td>
                    <td style="padding: 8px 0; text-align: right;">${formattedDate}</td>
                  </tr>
                  ${isPickup ? `
                  <tr>
                    <td style="padding: 8px 0; color: #666; vertical-align: top;">Pickup Location:</td>
                    <td style="padding: 8px 0; text-align: right;">${PICKUP_LOCATION}</td>
                  </tr>
                  <tr>
                    <td colspan="2" style="padding: 8px 0; color: #666; font-style: italic; font-size: 13px;">${PICKUP_HOURS}</td>
                  </tr>
                  ` : `
                  <tr>
...
export async function sendStatusUpdateEmail({
  ...
  if (status === 'ready') {
    if (isPickup) {
      ...
      message = `Great news! Your beautiful ${flowerColorDisplay} Rose bouquet is ready for pickup.`;
    } else {
      subject = `Out for Delivery - ${orderNumber}`;
      headline = `On Its Way!`;
      message = `Great news! Your beautiful ${flowerColorDisplay} Rose bouquet is out for delivery.`;
    }
  } else if (status === 'delivered') {
    if (isPickup) {
      subject = `Order Complete - ${orderNumber}`;
      headline = `Thank You!`;
      message = `Your order has been picked up. We hope you love your beautiful ${flowerColorDisplay} Rose bouquet!`;
    } else {
      subject = `Delivered - ${orderNumber}`;
      headline = `Delivered!`;
      message = `Your beautiful ${flowerColorDisplay} Rose bouquet has been delivered. We hope it brings joy!`;
    }
  } else {
